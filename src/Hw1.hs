...
additivePersistence n = if n >= 10 then 1 + additivePersistence(sumList(digitsOfInt(n))) else 0
...
digitalRoot :: Int -> Int
digitalRoot n = if n >= 10 then digitalRoot(sumList(digitsOfInt(n))) else n
